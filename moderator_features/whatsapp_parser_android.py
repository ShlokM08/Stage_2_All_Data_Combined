...
def parse_whatsapp_chat(file_input: Union[str, TextIO]) -> list:
    """
    Parse WhatsApp chat from either a file path or a file-like object
    
    Args:
        file_input: Either a file path (str) or a file-like object (TextIO)
        
    Returns:
        list: Parsed chat data
    """
    ...
    try:
        ...
        for line in file_obj:
            try:
                ...
                # Handle system messages
                system_pattern = r'(\d{1,2}/\d{1,2}/\d{2,4}),\s*(\d{1,2}:\d{2}\s*(?:am|pm|AM|PM))\s*-\s*(.+)'
